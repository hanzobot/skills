...
if [ "$1" = "--dry-run" ]; then
  ...
  pgrep -f "botis gateway" 2>/dev/null | while read pid; do
    echo "  PID $pid: $(ps -p $pid -o command=)"
  done
  
  echo ""
  echo "Would start Gateway on port 18789:"
  echo "  cd /home/srose/botis"
  echo "  pnpm botis gateway --port 18790"
  echo ""
  echo "To actually restart, run without --dry-run"
  exit 0
fi

echo "🔌 Restarting Bot Gateway"
echo "================================"
echo ""

# Stop running gateway processes
echo "Stopping any running gateway processes..."
pkill -f "botis gateway" 2>/dev/null
sleep 2

# Check if processes are still running
REMAINING=$(pgrep -f "botis gateway" | wc -l)
if [ "$REMAINING" -gt 0 ]; then
  echo "Force stopping remaining processes..."
  pkill -9 -f "botis gateway" 2>/dev/null
  sleep 1
fi
...
# Start in background
pnpm botis gateway --port 18789 > /dev/null 2>&1 &
GATEWAY_PID=$!
...
if ps -p $GATEWAY_PID > /dev/null 2>&1; then
  ...
  echo ""
  echo "Check logs:"
  echo "  cd /home/srose/botis && pnpm botis gateway --port 18789"
  exit 1
fi
