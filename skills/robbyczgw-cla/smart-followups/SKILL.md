...
Generate contextual follow-up suggestions for Bot conversations.
...
> **Note:** This is a keyword the agent recognizes, not a registered `/slash` command. Bot skills are guidance docs that tell the agent how to respond.
...
**Default:** Uses Bot's existing auth — same login and model as your current chat.
...
| `apiKey` | — | API key for non-bot providers |
...
3. Bot generates 3 contextual questions (using current model/auth)
4. Formatted as buttons or text based on channel
5. User clicks button or replies with number
6. Bot answers that question
