...
Robby wanted to bring this feature to Bot as a standalone skill that could work across multiple messaging channels.
...
#### 2. Bot Native Auth as Default

**Decision:** Use Bot's existing authentication by default, not separate API keys
...
**Implementation:**
- Default `provider: "bot"` uses current session's model and auth
...
The skill works by returning a prompt to Bot's agent system:
...
Handler returns agent-prompt type response
    ↓
Bot agent generates follow-ups using current model/auth
    ↓
...
- Uses Bot's existing infrastructure
- Inherits session model and authentication
- Consistent with Bot's architecture
...
|------|---------|----------|
| `README.md` | Public documentation | Users, Skills |
| `SKILL.md` | Bot skill manifest | Bot |
| `FAQ.md` | Common questions | Users |
| `INTERNAL.md` | This file - dev notes | Developers |
| `handler.js` | Command handler | Bot |
...
- [ ] Integration with Bot memory (suggest based on past conversations)
...
| Claude Sonnet 4.5 | ~$0.003 | Default for Bot |
