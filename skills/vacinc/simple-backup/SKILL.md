...
metadata: {"bot":{"emoji":"💾","requires":{"bins":["rclone","gpg","tar"]}}}
---

# Simple Backup

A robust backup script that:
1.  **Stages:** Copies `~/bot` (workspace), `~/.bot` (state), and `skills/`.
...
2.  **Password:** Set the encryption password:
    ...
    *   File: `~/.bot/credentials/backup.key`
...
|----------|---------|-------------|
| `BACKUP_ROOT` | `~/bot/BACKUPS` | Local storage location |
