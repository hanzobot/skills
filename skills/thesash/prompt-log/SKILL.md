---
name: prompt-log
description: Extract conversation transcripts from AI coding session logs (Bot, Claude Code, Codex). Use when asked to export prompt history, session logs, or transcripts from .jsonl session files.
---
...
- **Session file**: A `.jsonl` session log from Bot, Claude Code, or Codex.
...
scripts/extract.sh ~/.bot/agents/main/sessions/123.jsonl --output my-transcript.md
```
