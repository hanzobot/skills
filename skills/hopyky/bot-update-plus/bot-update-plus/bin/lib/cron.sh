...
install_cron() {
  ...
  log_info "To remove, run: bot-update-plus uninstall-cron"
...
uninstall_cron() {
  if ! crontab -l 2>/dev/null | grep -q "bot-update-plus"; then
    log_warning "No cron job found for bot-update-plus"
    return 1
  fi

  log_info "Removing cron job..."

  # Remove lines containing bot-update-plus
  crontab -l 2>/dev/null | grep -v "bot-update-plus" | grep -v "Bot Update Plus" | crontab -
