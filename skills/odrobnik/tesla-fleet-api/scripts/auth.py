...
State layout (default dir: ~/.bot/tesla-fleet-api):
...
def cmd_login(rt: TeslaRuntime, dir_path: str) -> int:
    if not rt.client_id:
        print("Missing client_id (set TESLA_CLIENT_ID in ~/.bot/tesla-fleet-api/.env)", file=sys.stderr)
        return 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--dir", default=default_dir(), help="Config directory (default: ~/.bot/tesla-fleet-api)")
